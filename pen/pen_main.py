...
from tkinter import Tk, filedialog  # 创建简单的GUI窗口
import os
...
def plot_results(fig, ax, characters, xlim, ylim, title_suffix, highlight_pairs=None):
    """
    在给定坐标轴上绘制多个字符，每个字符内的笔画用同一种颜色。
    highlight_pairs: 要高亮的原始字符索引对
    被高亮的字符（原始索引）用红色绘制，其余用随机颜色
    """
    # 生成随机颜色列表，种子固定
    ...
    for idx, character in enumerate(characters):
        if idx in highlight_indices:
            ...
            linewidth = 1.0
        # 对于字符内的每个笔画，用简单线图而非压力映射绘制，颜色统一为字符颜色
...
"""
数据输入->预处理->笔画分割->字符聚类->refine合并->可视化对比
"""
def main():
    ...
    # --- 计算坐标范围（从字典中提取）---
    # 从原始数据中获取所有点的最小/最大 X、Y，加上边距，确定 xlim 和 ylim
    try:
        x_coords = raw_data['x']
        y_coords = raw_data['y']
