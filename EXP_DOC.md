...
```powershell
python experiments/run_experiments.py --feature_matrix data/feature/all.csv --gate_decisions data/feature/gate_decisions.csv --out_dir results/
```


#### 4.6.2 严格防泄漏的正确实现

如果使用`feature_matrix.csv` ，用**全部可分析样本**（含测试样本）的统计量算的 z-score。测试样本参与了自己的归一化——数据泄漏。

**正确的 LOSO 需要根据原始特征all.csv，每折重新归一化**：
