...
> **代码仓库**：https://github.com/wyysw/handwriting-dysgraphia-analysis
...
通过分析儿童在三款手写游戏中的绘制轨迹，判断该绘制是否由书写障碍（Dysgraphia）儿童产生（**二分类**问题）。强调**方法论贡献**：同时兼顾"游戏结果"（游戏完成质量F）和"游戏过程"（运动控制C），设计跨游戏通用的轻量级分类器和特征框架，从特征抽取到落地分类（这个分类器不是只能训练单一游戏，而是对这三个游戏都行之有效；分类器只是为了表征我的工作从一个纯工程项的抽取特征变成了有落地的分类）。
...
- **txt轨迹格式**：每行 `x y pressure`，前 3 行为文件头（`eink` / `1` / `0,0`）需跳过，无时间戳
- **压力**：> 0 为落笔，= 0 为抬笔

- **从txt到png的映射方法**：features/mask_utils.py, features/trajectory_io.py
...
阶段 1 （已完成）  对称游戏特征提取器
   ↓
阶段 2（已完成）   方形迷宫游戏特征提取器
   ↓
阶段 3（已完成）   圆形迷宫游戏特征提取器
   ↓
阶段 4（已完成）   特征汇总与处理
   ↓
阶段 5（已完成）   分类器设计与实现
...
1. **矩形外框定位**：对灰度图做阈值反转 + 膨胀，再用找到最大且形状近似矩形的轮廓，得到迷宫的 ROI 矩形。
2. **线条提取**：CLAHE 增强后，用 **Blackhat 形态学变换** 作为主通道提取暗线，再用灰度阈值做弱线补充，融合得到初步线条掩码。
3. **角落装饰清理**：在右上角与左下角两个固定子区域内，通过连通域的面积、长宽比、填充率等形状特征，区分"线条型"和"色块型"组件，只删除色块而保留线条。
4. **外框重建**：在 ROI 内重绘矩形外框，修复上一步误删的角落边框。
5. **孤立短线删除**：清除阈值以下噪点级短线段。
6. **回填全图**：ROI 内的掩码通过放回 1201×1601 画布。

#### 4.0.2 对称游戏提取

1. **左右忽略区预处理**：将测评图左右边缘的题目说明区涂白防止干扰后续提取。
2. **蓝色折线提取**：通过HSV阈值（H∈[90,140]）提取原图的蓝色折线部分。
3. **外框提取**：用灰度反转阈值找到大矩形外框。
4. **原始网格 + 虚线提取**：在 ROI 内用提取浅灰色候选像素，再用水平/垂直形态学开运算分离出网格线，并用行积分峰值定位虚线。
5. **网格补全**：对网格做列和/行和分析，聚类确定竖线/横线位置，再删除与外框过近的首尾"伪辅助线"；最后当发现某个相邻间距明显偏大时，在中间补一条中轴线。
6. **合并辅助线框**：把外框+补全网格+虚线合并。

#### 4.0.3 圆形迷宫提取

1. **透明通道处理**：若输入为 RGBA，先把`alpha == 0`的像素置为黑色。
2. **白色画布定位**：找到最大白色连通域作为 ROI。
3. **迷宫线提取**：CLAHE 增强对比度；双阈值（强黑+弱灰）融合；识别彩色箭头，膨胀后从线条掩码中扣除；轻微水平/垂直闭运算保证曲线连续；去除面积小的噪点。
4. **画布边缘清理**：抹掉 ROI 最外2像素（阈值法易把白色画布边识别为线）。
5. **孤立短线删除**：与方形迷宫共用逻辑。
6. **回填全图**。
...
**目的**：对一个对称游戏样本，提取 7 个标准化特征（F1–F4, C1–C3），输出为 JSON。对称游戏的**核心特征计算程序**。
...
**F1：翻转容差 F1（0-1，越大越好）**：使用容差膨胀后的 F1 分数，衡量翻转后的用户笔迹与目标图形的重叠程度。
...
- 对称轴为**水平线** axis_y
- 用户应在下半画布（y > axis_y）书写，越界到上半画布的像素会被额外惩罚
...
- 在 sym_feature_extractor 里保留 pressure 维度
...
extract_sym_features(txt_path, png_path, blue_mask_path, helper_mask_path, out_json_path)
  ...
  ├─ 轨迹→笔段（调用split_into_strokes_simple）
...
- 生成一张可视化叠加图（用户翻转笔迹 + 蓝图 + 关键点命中情况），目视确认坐标对齐无误
...
#### 4.2.1 通道几何 ：`features/maze_geometry.py`

仅依赖 maze_mask.png，与手写轨迹样本无关。
...
openings = frame_ring ∩ channel_mask    # 框上可走的像素
...
骨架像素约 10³ 量级。对这张迷宫预估解路径长度在 3000–5000 像素。
...
使用 `channel_dist`（违规像素到最近通道的距离）自然体现了"错得多远就罚多重"——穿墙一格和画到纸外的惩罚力度自动拉开。不额外加硬惩罚系数，保持实现简单。
...
除转角处以外用户的笔迹理想情况下都是**横向或纵向直线段**，Hough 能稳定地把这些"用户应该画成直线"的部分识别出来。

**实现：把对称游戏模块中的C1计算实现直接复用，只换 `hough_segments` 的来源——使用手写笔迹的 Hough 线段。**

```
user_mask = render_trajectory_using_reference_bbox(txt, png, canvas_hw, line_thickness=3)
user_hough_segments = extract_segments_from_hough(user_mask)   # 复用_extract_target_segments_from_hough
...
`_extract_target_segments_from_hough`已经包含了按角度分类（h/v/diag_pos/diag_neg）→ 聚合共线片段 → 去除过短段的完整流程。


| 参数 |  设计理由 |
|---|---|
| `threshold` |  用户笔迹有抖动 |
| `minLineLength` | 方形迷宫的直线段较长，排除短过渡段 |
| `maxLineGap` | 用户笔迹有小间隙 |


**边界情况**：正常样本应有多条合并后的线段；对于极端抖动的障碍样本：Hough 找不到足够线段时，可能只有少数几条或零条。这种情况下"投影命中数 total"会很小，C1 = bad/total 不稳定。在 meta 里额外记录 `projected_fraction = total / num_user_points`，作为"线段一致性程度"的辅助诊断。核心公式与 sym 完全一致（`C1 = bad / projected`），满足"三游戏统一"的方法论约束。
...
笔段内部的压力全 > 0，公式与对称游戏相同。


#### 4.2.3 公共库：`features/stroke_utils.py`

C1/C2/C3 + 轨迹映射 + Hough 等通用逻辑，通用逻辑即对于三个游戏的实现都完全一致的部分。

| 公共函数 |  用途 |
|---|---|
| `load_strokes_with_pressure(txt)` |  读 txt，返回含 pressure 的笔段 dict 列表 |
| `map_strokes_to_canvas(strokes, png_path, canvas_hw)` |  bbox 映射 |
| `render_strokes_to_mask(strokes_mapped, canvas_hw, thickness)` |  笔段渲染 |
| `extract_segments_from_hough(mask, **hough_params)` | Hough→共线合并（对称和迷宫复用） |
| `compute_C1_jitter_ratio(strokes, segments, …)` |  C1 |
| `compute_C2_short_stroke_ratio(strokes, canvas_hw, …)` |  C2 |
| `compute_C3_pressure_cv(strokes_wp, …)` | C3 |
...
```powershell
python features/maze_feature_extractor.py --txt data/raw/maze/{id}.txt --png data/raw/maze/{id}.png --mask data/shape_out/maze_mask.png --out data/feature/maze/{id}.json --vis_dir data/feature/maze/vis_{id} --sample_id {id}
```
...
两迷宫复用**同一套提取器与特征定义**，通过 `game_type='circle'` 区分几何分支。所有差异集中在 `maze_geometry.py` 的几何构造步骤；**F1–F4 零改动，C2/C3 零改动**。C1 是唯一存在方案分歧的地方（见 4.3.5）。
...
#### 4.3.3 程序：`features/maze_geometry.py` 圆形分支

`game_type='circle'` 时调用`_build_circle_geometry`。`MazeGeometry` 数据类新增三个可选字段记录圆形专属元信息。
...
wall_mask (来自 circle_mask.png)
  ...
  │
  ▼  ⑥ 入口/出口锚点
     角度扫描自动寻找缺口（circle_scan_entry=True）
  │
...
**步骤③：通道掩码**（调用已有 `_build_channel_mask`）

**步骤④：连通域过滤（圆形专用）**

中心装饰小圆围出了一个封闭区域，该区域在 `channel_mask_raw` 中形成一个与主环系统不连通的孤立 blob（面积约 远小于主环系统）。保留面积最大的连通域即可将其过滤。
...
扫描 channel_mask，找连续 True 角度段，取两个最大段的角度质心。支持外部显式传入 `entry_xy` / `exit_xy` 覆盖（与方形一致）。
...
**MazeGeometry 数据类新增字段**：方形迷宫调用时三个字段均为 `None`，对现有代码无影响。
...
def compute_C1_jitter_ratio_skeleton(
    ...
    # KDTree 加速（骨架点≈3000–5000，笔迹点≈8000）
...
方形迷宫原有参数不变，新增以下圆形专用参数（均有默认值，方形迷宫调用时完全忽略）：
...
```json
{
  ...
  "meta": {
    ...
    "C1_method": "skeleton_dist",   ← 方形 Hough时为hough_user
...
**输入**:表头为{sample_id,game,label,F1,F2,F3,F4,C1,C2,C3}的表格
...
**输出 `gate_decisions.csv`**：追加 is_unanalyzable / triggered_rules / F3_zscore / F4_zscore 列
...
2. **裁剪范围 `(-3, +6)` 的理由**：左侧 -3 对应"比正常还正常"，继续往左的信息价值已饱和；右侧 +6 保留严重异常样本的强信号。如果发现大量样本被裁到 +6，说明 MAD 估计过小，需要调整下限。
3. **乱画样本不参与归一化的拟合，不进入最终的特征矩阵**——在阶段6被赋予 prob=1.0。
...
**流程**：
```
1. 读取包括特征值、label标注的样本总表
2. 调用 gate_unanalyzable → 得到 is_unanalyzable 标记
3. 筛选 {可分析 且 label=0} 样本 → fit_normalize_stats
4. 对 {全部可分析样本} 调用 apply_normalize
5. 输出三个文件：
   - feature_matrix.csv       # 可分析样本的z-score矩阵（主分类器输入）
   - gate_decisions.csv       # 全部样本的门控结果（阶段6重并入用）
   - normalize_stats.json     # 归一化统计量（调试 + 报告用）
```
...
- label=0 样本的 z 值应大致在 [-2, +2] 范围内；
    ...
    结果: [ 0.28  0.48  0.5   1.25 -0.06  0.56  0.03]

- label=1 样本应在至少 1-2 个特征上出现 z > 2 的明显偏离；

    结果: [1.27 2.24 2.88 4.07 0.4  1.03 0.49]
...
**边界值 0.3 的含义**：每个特征权重在先验值的 ±30% 内微调。如果数据强烈主张某特征的权重"应该"更高或更低，优化结果会贴到边界上——这或许是一个可报告的发现。
...
**C 参数的选择**：在阶段6的LOSO循环内嵌套一个小的网格搜索 `C ∈ {0.1, 0.3, 1.0, 3.0}`，用内层LOO选最优。考虑到样本量小，嵌套CV会让评估时间变长但结果更诚实。
...
**是否能证明"非线性模型并未显著提升"，以及支持"简单模型已够"的方法论主张**？如果 M4 显著优于 M3，说明决策边界非线性，需要反思 LR 是否足够。
...
上面的流程有一个严重问题：`feature_matrix.csv` 是用**全部可分析样本**（含测试样本）的统计量算的 z-score。测试样本参与了自己的归一化——数据泄漏。
...
model                                            
M2_PurePrior 0.7720 0.8319  0.8696         0.0027
M1_SemiPrior 0.7195 0.8085  0.8444         0.4676
M3_L2LR      0.7110 0.8204  0.8352         0.2957
M4_RF        0.7811 0.8892  0.8395         0.6000
...
**关键对比叙事**：
- M2 → M1： Δ AUROC = +0.0524  数据微调的边际价值
- M1 → M3： Δ AUROC = -0.0610  软约束（权重盒约束）vs 硬约束（L2正则）
- M3 → M4： Δ AUROC = +0.0701  非线性模型的增益
- 若 M3 ≈ M4：**"本任务接近线性可分，简单模型已足够"**
...
M2_PurePrior         1.000           NaN      NaN    NaN
M1_SemiPrior           NaN         1.000    0.929  0.821
M3_L2LR                NaN         0.929    1.000  0.929
M4_RF                  NaN         0.821    0.929  1.000
```

### 4.6.6 SHAP值："experiments/shap_m3.py"

对 M3 做样本级的 SHAP 解释，展示具体某个样本为什么被判为异常。
