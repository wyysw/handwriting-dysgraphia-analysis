...
分类器逻辑集成在 `classify.py` 中，不作为独立模块。
...
#### 5.3.5 程序 B：`features/maze_feature_extractor.py`
...
| 步骤 | 方形迷宫 |
|---|---|---|
| 通道掩码来源 | `maze_mask.png` |
| 正确路径提取 | 用形态学腐蚀获取"通道内部"（迷宫墙是线条，腐蚀后中间空白即通道） |
| F2 关键点定义 | 路径骨架上的岔路口/转角 |
| 坐标对齐 | 同对称游戏 | 
