...
| 游戏 | 任务描述 | 参考图像文件 |
|---|---|---|
| 对称（Symmetry） | 依据对称轴，将左侧蓝色图形镜像补画到右侧 | `sym_blue_mask.png`, `sym_helper_mask.png` |
...
**目的**：对一个对称游戏样本，提取 7 个标准化特征（F1–F4, C1–C3），输出为 JSON。这是对称游戏的**核心特征计算程序**。不做加权、不做打分。加权归一化留给阶段 3。
...
**不复用**的：`analyze_line_control`、`run_stage1`、`tolerant_f1`、`keypoint_coverage_score` —— 这些是带打分逻辑的包装，阶段1会用更裸的指标替代。
...
- 对应的 `.png` 文件（用于坐标参照）
- `sym_blue_mask.png` 和 `sym_helper_mask.png`（模块 1 的输出）

**测试方法**：
1. 对 `s1.txt`（已有样本）运行，检查 JSON 是否正常输出，各特征值是否在合理范围
