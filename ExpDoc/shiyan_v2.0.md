...
- **压力**：> 0 为落笔，= 0 为抬笔
- **无时间戳**（速度、耗时类特征全部不可用）
- **样本量与标注**：参考labels.csv。（sym样本包含5个正常样本和3个书写障碍样本，circle包含9个正常样本和12个书写障碍样本，maze包含14个正常样本和20个书写障碍样本。）
...
阶段 3（已完成）   迷宫游戏特征提取器（圆形，与方形迷宫共用基类）
   ↓
阶段 4（待修改）   特征汇总 + 归一化管道（输出标准化特征矩阵 CSV）（待确认）
   ↓
阶段 5（待修改）   分类器设计与实现（方案 A 加权打分 / 方案 B 双轴偏离 / 方案 C 机器学习）（不一定是这三种）
   ↓
阶段 6（待修改）   实验评估
```
...
```
python features/sym_feature_extractor.py --txt data/raw/sym/{id}.txt --png data/raw/sym/{id}.png --blue data/shape_out/sym_blue_mask --helper data/shape_out/sym_helper_mask.png  --out output_sym/extract/{id}.json --vis vis_{id}.png
```
...
F1–F4 直接复用 `maze_feature_extractor.py` 中的四个函数。下表仅列出关键行为：
...
待确认：归一化参数估计方法！以下设计只能作为参考，需要再次考量是否合适！
