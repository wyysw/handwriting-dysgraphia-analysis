...
3. 用 matplotlib 两组并排展示：
   - 第一组：原始迷宫 vs 迷宫线条
   - 第二组：原始对称游戏 vs 对称图形（蓝色折线） vs 辅助线框

> 若要将圆形迷宫也纳入调度，只需仿照 `run_maze()` 新增 `run_circle()`，调用 `final_shape_circle.extract_maze("data/36circle.png", "./output_circle/shape_circle")`，并在 `main()` 中 `show_group` 一次即可。
...
- `opencv-python`（`cv2`）
- `numpy`
- `matplotlib`（仅 `shape.py` 可视化使用，提取模块本身不依赖）
...
# 方形迷宫
migong.extract_maze("data/34migong.png", "./output_maze/shape_maze")

# 对称图
sym.extract_symmetry(
    image_path="data/35duichen.png",
    out_dir="./output_sym/shape_sym",
...
# 圆形迷宫
circle.extract_maze("data/36circle.png", "./output_circle/shape_circle")
```

运行完毕后，各自 `out_dir` 下的 `*_mask.png` 即为下一阶段算法可直接使用的标准参考结构。