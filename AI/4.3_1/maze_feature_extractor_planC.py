"""
features/maze_feature_extractor.py  ── 方案 C 版本

阶段 2/3 — 迷宫游戏特征提取主程序，支持方形（maze）和圆形（circle）迷宫。

方案 C（本文件采用）：
  - 两种 game_type 均使用骨架距离法 C1（compute_C1_skeleton_distance_ratio）。
  - 与特征定义总表"C1：三游戏统一"的表述完全对齐：
      "笔迹到参考骨架的残差越界比例"
  - 代价：方形迷宫 C1 需重新运行（与阶段 2 的 Hough-on-user 结果不同）。

方案 A（见 maze_feature_extractor_planA.py）：
  - game_type='circle' 用骨架距离，game_type='maze' 保留 Hough-on-user。
...
特征定义（方形/圆形完全一致）：
    ...
    C1 = bad / total（骨架距离残差超 jitter_tol 的点比例）       ↓
    C2 = 短笔段总长/所有笔段总长                                ↓
    C3 = σ(pressure) / μ(pressure)                              ↓

依赖：
    - features.maze_geometry   ：通道几何 + 解路径（v2.1，支持 game_type）
    - features.stroke_utils    ：C1/C2/C3 + 轨迹 IO + 渲染（公共库）
...
from features.stroke_utils import (
    compute_C1_skeleton_distance_ratio,     # 方案 C：两种迷宫均用骨架距离法
...
def compute_F2_keypoint_hit_rate(
    ...
    命中：该点到用户笔迹的距离 <= hit_radius。
    返回 (F2, hit_count, total_K, sample_pts)。
    """
...
def compute_F4_offchannel_ratio(
    ...
    if n_user == 0:
        return 0.0
    return float(int((user_pts & (channel_mask == 0)).sum())) / float(n_user)
...
def extract_maze_features(
    ...
    *,
    ...
    sample_step: float = 40.0,
    hit_radius:  float = 12.0,
    # C1 参数（骨架距离法，方案 C 两种迷宫共用）
    jitter_tol: float = 3.0,
    channel_half_width_C1: float = 28.0,
    ...
    方案 C 的 C1 规则：
        game_type='maze' 和 'circle' 均调用 compute_C1_skeleton_distance_ratio，
        使用 channel_skeleton 作为参考，测量笔迹点到骨架的距离残差。
        这与特征定义总表（三游戏统一）完全对齐。
    """
    ...
    if png_path is not None and os.path.exists(png_path):
        ...
        if not inner_bbox_fallback:
            raise FileNotFoundError(
                f"png_path 不存在或未提供（inner_bbox_fallback=False）: {png_path}"
            )
        mapped_strokes = map_strokes_to_canvas(
    ...
    # ---------- 6. C1（方案 C：两种迷宫统一用骨架距离法）----------
    # ┌─────────────────────────────────────────────────────────────────────────┐
    # │ 方案 C                                                                  │
    # │   无论 game_type，C1 均用骨架距离法，与三游戏统一的特征定义对齐        │
    # │   与方案 A 的差别：方形迷宫的 C1 从 Hough-on-user 改为骨架距离         │
    # └─────────────────────────────────────────────────────────────────────────┘
    ...
    result: Dict[str, Any] = {
        ...
        "meta": {
            ...
            "num_user_hough_segments": 0,   # 方案C不走Hough，恒为0
...
def _vis_C1_skeleton(
    ...
    """
    C1 骨架距离叠加图（方案 C：两种迷宫共用）：
      - 灰  : 用户笔迹
      - 青  : 通道骨架（参考中心线）
      - 黄  : 抖动点（d <= channel_half_width 且 d > jitter_tol）
    """
    h, w = user_mask.shape[:2]
    vis = np.zeros((h, w, 3), dtype=np.uint8)
    vis[user_mask > 0]        = (160, 160, 160)
    vis[channel_skeleton > 0] = (200, 180, 0)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="迷宫游戏特征提取器（方案C：两种迷宫统一用骨架C1）")
