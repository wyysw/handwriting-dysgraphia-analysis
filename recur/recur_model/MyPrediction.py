...
class MonteCarloPredictor:
    ...
    def __init__(self, models_dir='./outputs'):
        ...
        self.models = {}    # 存储加载的多个模型
        self.label_encoders = {}    # 类别特征的标签编码器
        self.feature_columns = []   # 训练时使用的所有特征列名
        self.categorical_columns = []   # 类别特征列名
    ...
    def extract_features(self, child_data_row: pd.Series) -> np.ndarray:
        ...
        return np.array(features)   # 将特征列表转换为特征数组并返回
    ...
    def monte_carlo_forecast(self,
        ...
        Args:
            ...
            time_interval: 每一步的时间间隔（年）
            model_name: 指定使用的模型名称，用于模拟观测误差
            noise_std: 噪声标准差
            noise_growth_rate: 噪声随预测步数增长的比例因子
            model_uncertainty_factor: 模型不确定性系数,模拟模型本身的误差
        """
        ...
        print(
            f"Child info: 性别={child_row.get('性别', 'N/A')}, 矫正方式={child_row.get('矫正方式', 'N/A')}, 筛查区={child_row.get('筛查区', 'N/A')}")
        ...
        # 找到关键特征target_age 和 target_sph_r在特征向量中的索引位置
        age_idx = None
        target_idx = None
        ...
        # 找到所有历史年龄和屈光度特征在特征向量中的索引，用于滑动窗口更新
        ...
        return {
            ...
            'time_points': [last_age + time_interval * (i) for i in range(n_steps)],
            'mean_pred': mean_pred,
...
#     example_child_id = '110103201305231813'  # 替换为实际的child_id
#
#     try:
...
def main():
    ...
    try:
        ...
        for index, row in data.iterrows():
            ...
            print(f"预测完成! Child ID: {child_id}")

            # 保存结果为图像文件（这里假设forecast_result是一个可以绘制成图像的对象）
            output_file_path = os.path.join(output_dir, f'result_{child_id}.png')
            forecast_result.plot().savefig(output_file_path)  # 这里假设forecast_result有一个plot方法
