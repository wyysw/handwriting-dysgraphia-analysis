...
def load_json_features(game, sample_id):
    json_path = f"data/feature/{game}/{sample_id}.json"
...
def main():
    ...
    # === 提取指定 sample_id 的记录 ===
    target_ids = ["s4","c3","c6","c18","l10","l13","l15","l16","l17","l29"]
    df_luanhua = df_all[df_all["sample_id"].isin(target_ids)]
    luanhua_path = "data/feature/luanhua.csv"
    df_luanhua.to_csv(luanhua_path, index=False, encoding="utf-8-sig")
    print(f"[Saved] {luanhua_path}")
