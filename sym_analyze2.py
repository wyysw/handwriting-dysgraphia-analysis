...
if __name__ == "__main__":
    base = "data/sym"
    out_dir = "output_sym/stage1_2_3_output"
    res = run_stage1(
    ...
    # 输出各模块百分制得分
    import json, os
    metrics_file = os.path.join(out_dir, "stage1_metrics.json")
    if os.path.exists(metrics_file):
        with open(metrics_file, "r", encoding="utf-8") as f:
            meta = json.load(f)
        ms = meta.get("module_scores", {})
        print("shape_score:", ms.get("shape_score", None))
        print("size_score:", ms.get("size_score", None))
        print("keypoint_score:", ms.get("keypoint_score", None))
        print("line_control_score:", ms.get("line_control_score", None))
